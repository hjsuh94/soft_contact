...
class FingerSimulator():
    ...
    State definition: [qx_b, qy_b, theta_b, q1_l, q1_r]
    Input definition: [u1_l, u1_r]
    ...
    def __init__(self, gui=True):
        ...
        self.params = {'m': 0.1,  # mass of each finger
                       ...
                       'cb': 10.0, # damping on the manipuland
                       ...
                       'l': 125e-3, # length of each finger
    ...
    def update_params(self):
        ...
        self.params['alpha'] = self.params['I'] + 1. * self.params['m'] * (
            self.params['rl'] ** 2.0)
    ...
    def jacobian(self, x):
        ...
        q1,_ = x # Unpack Joints

        J = np.array([-self.params['l'] * np.sin(q1), self.params['l'] * np.cos(q1)])
    ...
    def manipulator_params(self, x):
        ...
        q1,q1dot = x # Unpack joints

        M = np.array(self.params['alpha']).astype(np.double)

        C = np.array(0).astype(np.double)

        G = np.array(self.params['m'] * self.params['rl'] * self.params['g'] * np.cos(q1)).astype(np.double)
    ...
    def unpack_states(self, x):
        ...
        px_b, py_b, theta_b, q1_l, q1_r = x[0:self.nq]
        vx_b, vy_b, omega_b, q1dot_l, q1dot_r = x[self.nq:(2*self.nq)]

        x_b = np.array([px_b, py_b, theta_b, vx_b, vy_b, omega_b])
        x_l = np.array([q1_l, q1dot_l])
        x_r = np.array([q1_r, q1dot_r])
    ...
    def pack_states(self, x_b, x_l, x_r):
        ...
        x = np.array([px_b, py_b, theta_b, q1_l, q1_r,
                      vx_b, vy_b, omega_b, q1dot_l, q1dot_r])
    ...
    def step(self, x, u):
        ...
        x_b_new[3:5] = x_b[3:5] + self.params['h'] * (1. / self.params['mb']) * (
            lambda_l + lambda_r - self.params['cb'] * x_b[3:5])
        x_b_new[4] = x_b_new[4] - self.params['mb'] * self.params['g']
        x_b_new[5] = x_b[5] + self.params['h'] * (1./self.params['Ib']) * 0.
        
        # Update left manipulator velocity
        Ml, Cl, Gl = self.manipulator_params(x_l)
        Jl = self.jacobian(x_l)

        x_l_new[1] = x_l[1] + self.params['h'] * (1./ Ml) * (
            Gl + tau1_l
            -np.dot(np.transpose(Jl), lambda_l))

        # Update right manipulator velocity 
        Mr, Cr, Gr = self.manipulator_params(x_r)
        Jr = self.jacobian(x_r)

        x_r_new[1] = x_r[1] + self.params['h'] * (1./ Mr) * (
            Gr + tau1_r
            -np.dot(np.transpose(Jr), lambda_r))

        # Propagate positions---------------------------------------------------
        x_b_new[0:3] = x_b[0:3] + self.params['h'] * x_b_new[3:6]
        x_l_new[0] = x_l[0] + self.params['h'] * x_l_new[1]
        x_r_new[0] = x_r[0] + self.params['h'] * x_r_new[1]

        # Repack states and return----------------------------------------------

        return self.pack_states(x_b_new, x_l_new, x_r_new)
    ...
    def render(self, x):
        ...
        self.draw_circle(p1_l, self.params['w'] / 2, 'black', 'blue')
        self.draw_circle(p1_r, self.params['w'] / 2, 'black', 'blue')
        ...
        self.draw_circle_image(draw, p1_l, self.params['w'] / 2, 'black', 'blue')
        self.draw_circle_image(draw, p1_r, self.params['w'] / 2, 'black', 'blue')
